  ...
  dockVessel(vesselId: string): void {
    this.subs.add(
      this.selectedVessel$.pipe(
        ...
        concatMap(updatedVessel => this.apiService.updateVessel(updatedVessel.nextStops[0], updatedVessel.id).pipe(
          map(() => updatedVessel)
        ))
      ).subscribe({
  ...
  unDockVessel(vesselId: string): void {
    this.subs.add(
      this.selectedVessel$.pipe(
        ...
        concatMap(updatedVessel => this.apiService.updateVessel(updatedVessel.stops[updatedVessel.stops.length - 1], updatedVessel.id).pipe(
          map(() => updatedVessel)
        ))
      ).subscribe({
        next: vessel => this._vessels$.next(this._vessels$.value.map(v => (vessel.id === v.id) ? ({ ...vessel }) : v)),
  ...
  editNextRoutes(vesselId: string, nextStops:Port[]): void {
    this.subs.add(
      this.selectedVessel$.pipe(
        ...
        next: vessel => this._vessels$.next(this._vessels$.value.map(v => (vesselId === v.id) ? ({ ...vessel }) : v)),
        error: err => this._error$.next(err),
